...
class Preview extends SurfaceView implements SurfaceHolder.Callback {	
	...
	public Preview(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
		Log.i("Preview", "Preview Constructor");
	}

	public Preview(Context context) {
		super(context);
		init();
		Log.i("Preview", "Preview Constructor1");
	}
	...
	public Bitmap getPic(int x, int y, int width, int height) {
		...
		if (b != null) {
			Log.i("Preview.getPic", "getPic() WxH:" + b.getWidth() + "x" + b.getHeight());
		} else {
			Log.i("Preview.getPic", "getPic(): Bitmap is null..");
		}
		yuvimage = null;
		outStream = null;
		System.gc();
	...
	private void updateBufferSize() {
		Log.i("Preview", "updateBufferSize");
		mBuffer = null;
		System.gc();
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		try {
			mCamera = Camera.open(Camera.CameraInfo.CAMERA_FACING_BACK); // WARNING: without permission in Manifest.xml, crashes
		}
	...
	public Parameters getCameraParameters(){
		Log.i("Preview", "getCameraParameters");
		return mCamera.getParameters();
	}

	public void setCameraFocus(AutoFocusCallback autoFocus){
		Log.i("Preview", "setCameraFocus");
